...
var svelteParsers = createSvelteParsers();
function createSvelteParsers() {
    try {
        var parsers = require('prettier-plugin-svelte').parsers;
        var sveltePreprocessor = require('./preprocessors/svelte-preprocessor').sveltePreprocessor;
    }
    catch (_a) {
        return {};
    }
    return {
        svelte: __assign(__assign({}, parsers.svelte), { preprocess: sveltePreprocessor }),
    };
}
var options = {
    importOrder: {
